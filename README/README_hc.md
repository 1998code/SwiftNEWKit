...
[English](../README.md) · [繁中](README_tc.md) · [简中](README_zh.md) · **粵語** · [日本語](README_ja.md) · [한국어](README_ko.md) · [Français](README_fr.md) · [Español](README_es.md)
...
已經獲 Apple 批准嘅 CarPlay host app，只可以喺更新內容直接符合獲批車載類別時用 `SwiftNEWCarPlayTemplateFactory`；一般產品 changelog 唔會自動符合資格。Scene lifecycle、內容同 entitlement 限制請睇 [CarPlay 整合指南](CARPLAY.md)。

## ✨ 功能

| 功能 | 起始版本 | 說明 |
|---------|:-----:|-------------|
| 🖋️ 自訂按鈕文字顏色 | 27.3 | 直接設定 `buttonTextColor` 或用 binding 控制，取代版本說明觸發按鈕同主要操作按鈕自動揀嘅黑色或白色文字 |
...
| 📥 獨立版本說明載入器 | 27.0 | `SwiftNEWReleaseNotesLoader` 唔使建立 SwiftUI view，都可以載入同解碼本地或遠端 JSON |
| 🎛️ 可設定按鈕圓角 | 27.0 | 直接設定 `buttonCornerRadius` 或用 binding 控制，令版本說明觸發按鈕同主要操作按鈕嘅外觀保持一致 |
| 🔎 歷史搜尋控制 | 27.0 | 可以喺每個 SwiftNEW 實例啟用 Search，並按標題 / 副標題 / 內文篩選目前同歷史版本說明 |
| 📝 可選說明文字 | 27.0 | `showDescription` 可直接設定或用 binding 控制，隱藏內文但保留圖示、標題同副標題 |
| 🌊 可重用 SwiftNEW 背景 | 27.0 | `SwiftNEWBackdrop` 可以喺自訂 SwiftUI 內容後面重用靜態 / 流動網格背景同裝飾特效 |
| 🌐 更多語言 | 6.6.0 | 版本說明而家支援德文、希伯來文、印尼文、俄文同越南文 |
| 🌗 自適應圖示樣式 | 6.6.0 | 圖示背景同字形漸層會喺深色模式自動改善對比 |
| 🫧 著色玻璃控制項 | 6.6.0 | 版本說明同主要操作按鈕喺 iOS 26 使用原生互動式著色玻璃 |
| 🔄 替代 App 圖示 | 6.6.0 | 將標題圖示同已選擇嘅替代 App 圖示保持同步 |
| 🎨 智能 App 圖示深色模式 | 6.6.0 | 自動將大範圍白色或中性色圖案轉成深色，同時保留強調色 |
| 📱 自動載入 App 圖示 | 6.6.0 | 尋找並顯示最適合目前裝置嘅內置 App 圖示 |
| ⬆️ 遠端更新畫面 | 6.5.0 | 用 `checkForUpdates` 啟用；遠端 JSON 有較新版本時，顯示更新畫面同可自訂文字嘅 App Store 操作按鈕 |
