...
| 🎛️ 可設定按鈕圓角 | 27.0 | 直接設定 `buttonCornerRadius` 或透過 binding 控制，讓版本說明觸發按鈕與主要操作按鈕維持一致外觀 |
| 🔎 歷史搜尋控制 | 27.0 | 可為每個 SwiftNEW 實例啟用 Search，並依標題 / 副標題 / 內文篩選目前與歷史版本說明 |
| 📝 可選說明文字 | 27.0 | `showDescription` 可直接設定或透過 binding 控制，隱藏內文但保留圖示、標題與副標題 |
| 🌊 可重用 SwiftNEW 背景 | 27.0 | `SwiftNEWBackdrop` 可在自訂 SwiftUI 內容後重用靜態 / 流動網格背景與裝飾特效 |
| 🫥 隱形觸發按鈕修正 | 27.0 | 設為 `size: "invisible"` 的觸發按鈕不再於畫面留下半透明玻璃效果 |
| 🌐 更多語言 | 6.6.0 | 版本說明現已支援德文、希伯來文、印尼文、俄文與越南文 |
| 🌗 自適應圖示樣式 | 6.6.0 | 圖示背景與字形漸層會在深色模式自動改善對比 |
| 🫧 著色玻璃控制項 | 6.6.0 | 版本說明與主要操作按鈕在 iOS 26 使用原生互動式著色玻璃 |
| 🔄 替代 App 圖示 | 6.6.0 | 將標題圖示與已選擇的替代 App 圖示保持同步 |
| 🎨 智慧 App 圖示深色模式 | 6.6.0 | 自動從大範圍白色或中性色圖案產生深色版本，同時保留強調色 |
| 📱 自動載入 App 圖示 | 6.6.0 | 尋找並顯示最適合目前裝置的內建 App 圖示 |
| ⬆️ 遠端更新畫面 | 6.5.0 | 透過 `checkForUpdates` 啟用；遠端 JSON 有較新版本時，顯示含可自訂文字 App Store 操作按鈕的更新畫面 |
