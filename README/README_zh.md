...
| 功能 | 起始版本 | 说明 |
|---------|:-----:|-------------|
| 🖋️ 自定义按钮文字颜色 | 27.3 | 直接设置 `buttonTextColor` 或通过 binding 控制，替换版本说明触发按钮与主要操作按钮自动选择的黑色或白色文字 |
...
| 📥 独立版本说明加载器 | 27.0 | `SwiftNEWReleaseNotesLoader` 无需创建 SwiftUI 视图，即可加载并解码本地或远程 JSON |
| 🎛️ 可配置按钮圆角 | 27.0 | 直接设置 `buttonCornerRadius` 或通过 binding 控制，让版本说明触发按钮与主要操作按钮保持一致外观 |
| 🔎 历史搜索控制 | 27.0 | 可为每个 SwiftNEW 实例启用 Search，并按标题 / 副标题 / 正文筛选当前与历史版本说明 |
| 📝 可选说明文本 | 27.0 | `showDescription` 可直接设置或通过 binding 控制，隐藏正文但保留图标、标题与副标题 |
| 🌊 可复用的 SwiftNEW 背景 | 27.0 | `SwiftNEWBackdrop` 可在自定义 SwiftUI 内容后复用静态 / 流动网格背景和装饰特效 |
| 🌐 更多语言 | 6.6.0 | 版本说明现已支持德语、希伯来语、印度尼西亚语、俄语和越南语 |
| 🌗 自适应图标样式 | 6.6.0 | 图标背景和字形渐变会在深色模式自动改善对比度 |
| 🫧 着色玻璃控件 | 6.6.0 | 版本说明和主要操作按钮在 iOS 26 使用原生交互式着色玻璃 |
| 🔄 替代 App 图标 | 6.6.0 | 将标题图标与已选择的替代 App 图标保持同步 |
| 🎨 智能 App 图标深色模式 | 6.6.0 | 自动从大面积白色或中性色图案生成深色版本，同时保留强调色 |
| 📱 自动载入 App 图标 | 6.6.0 | 查找并显示最适合当前设备的内置 App 图标 |
