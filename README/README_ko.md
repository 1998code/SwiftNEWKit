...
| 기능 | 도입 버전 | 설명 |
|---------|:-----:|-------------|
| 🖋️ 버튼 텍스트 색상 사용자 지정 | 27.3 | `buttonTextColor`를 직접 지정하거나 바인딩으로 전달해 릴리스 노트 버튼과 주요 동작 버튼의 자동 검정/흰색 텍스트를 변경 |
...
| 📥 독립형 릴리스 로더 | 27.0 | `SwiftNEWReleaseNotesLoader`로 SwiftUI 뷰를 만들지 않고 로컬 / 원격 JSON을 디코딩 |
| 🎛️ 버튼 모서리 반경 사용자 지정 | 27.0 | `buttonCornerRadius`를 직접 지정하거나 바인딩으로 전달해 주요 동작 버튼의 시각적 일관성 유지 |
| 🔎 버전 기록 검색 제어 | 27.0 | 인스턴스별로 검색을 활성화하고 제목 / 부제 / 본문을 기준으로 현재 및 이전 릴리스 노트를 필터링 |
| 📝 선택적 설명 표시 | 27.0 | `showDescription`을 직접 지정하거나 바인딩으로 전달해 아이콘·제목·부제는 유지하고 본문을 숨김 |
| 🌊 재사용 가능한 SwiftNEW 배경 | 27.0 | `SwiftNEWBackdrop`으로 사용자 지정 SwiftUI 콘텐츠 뒤에 정적 / 리퀴드 메시와 장식 효과를 추가 |
...
| 🔄 대체 앱 아이콘 | 6.6.0 | 헤더 아이콘을 선택된 대체 앱 아이콘과 동기화 |
| 🎨 스마트 앱 아이콘 다크 모드 | 6.6.0 | 넓은 흰색 또는 중성 영역을 어둡게 변환하면서 강조 색상은 유지 |
| 📱 앱 아이콘 자동 로딩 | 6.6.0 | 현재 기기에 가장 적합한 내장 앱 아이콘을 찾아 표시 |
