...
| 機能 | 追加バージョン | 説明 |
|---------|:-----:|-------------|
| 🖋️ ボタンの文字色をカスタマイズ | 27.3 | `buttonTextColor` を直接指定するか Binding で制御し、リリースノートボタンと主要アクションボタンの自動（黒または白）の文字色を上書き |
...
| 📥 単独で使えるリリースローダー | 27.0 | `SwiftNEWReleaseNotesLoader` で SwiftUI ビューを作成せずにローカル / リモート JSON をデコード |
| 🎛️ 設定可能なボタンの角丸 | 27.0 | `buttonCornerRadius` を直接指定するか Binding で制御し、主要アクションボタンの見た目を統一 |
| 🔎 履歴検索コントロール | 27.0 | インスタンスごとに検索を有効化し、現在 / 過去のリリースノートをタイトル / サブタイトル / 本文で絞り込み |
| 📝 説明文の任意表示 | 27.0 | `showDescription` を直接指定するか Binding で制御し、アイコン・タイトル・サブタイトルを残して本文を非表示 |
| 🌊 再利用可能な SwiftNEW 背景 | 27.0 | `SwiftNEWBackdrop` でカスタム SwiftUI コンテンツの背面に静止 / リキッドメッシュと装飾エフェクトを追加 |
...
| 🔄 代替 App アイコン | 6.6.0 | ヘッダーを選択中の代替 App アイコンと同期 |
| 🎨 App アイコンのスマートダークモード | 6.6.0 | 広い白色または無彩色の領域を暗く変換しながらアクセントカラーを維持 |
| 📱 App アイコンの自動読み込み | 6.6.0 | 現在のデバイスに最適な内蔵 App アイコンを検出して表示 |
